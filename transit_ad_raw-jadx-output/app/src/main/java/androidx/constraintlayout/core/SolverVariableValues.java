...
public class SolverVariableValues implements ArrayRow.ArrayRowVariables {
    ...
    public String toString() {
        String strF;
        String strF2;
        String strF3 = hashCode() + " { ";
        int i2 = this.mCount;
        for (int i3 = 0; i3 < i2; i3++) {
            SolverVariable variable = getVariable(i3);
            if (variable != null) {
                String str = strF3 + variable + " = " + getVariableValue(i3) + " ";
                int iIndexOf = indexOf(variable);
                String strF4 = s.f(str, "[p: ");
                if (this.previous[iIndexOf] != -1) {
                    StringBuilder sbH = b.h(strF4);
                    sbH.append(this.mCache.mIndexedVariables[this.variables[this.previous[iIndexOf]]]);
                    strF = sbH.toString();
                } else {
                    strF = s.f(strF4, "none");
                }
                String strF5 = s.f(strF, ", n: ");
                if (this.next[iIndexOf] != -1) {
                    StringBuilder sbH2 = b.h(strF5);
                    sbH2.append(this.mCache.mIndexedVariables[this.variables[this.next[iIndexOf]]]);
                    strF2 = sbH2.toString();
                } else {
                    strF2 = s.f(strF5, "none");
                }
                strF3 = s.f(strF2, "]");
            }
        }
    ...
    public float use(ArrayRow arrayRow, boolean z) {
        ...
        while (i2 < currentSize) {
            ...
            if (i4 != -1) {
                add(this.mCache.mIndexedVariables[i4], solverVariableValues.values[i3] * f, z);
                i2++;
            }
            i3++;
        }
