...
class BrowserActionsFallbackMenuAdapter extends BaseAdapter {
    ...
    public View getView(int i2, View view, ViewGroup viewGroup) {
        ...
        if (browserActionItem.getIconId() != 0) {
            ...
            final l<Bitmap> lVarLoadBitmap = BrowserServiceFileProvider.loadBitmap(this.mContext.getContentResolver(), browserActionItem.getIconUri());
            lVarLoadBitmap.addListener(new Runnable() { // from class: androidx.browser.browseractions.BrowserActionsFallbackMenuAdapter.1
                @Override // java.lang.Runnable
                public void run() {
                    Bitmap bitmap;
                    if (TextUtils.equals(title, viewHolderItem.mText.getText())) {
                        try {
                            bitmap = (Bitmap) lVarLoadBitmap.get();
