...
public final class TextLayoutHelperKt {
    ...
    public static final boolean m899canReuse7_7YC6M(TextLayoutResult textLayoutResult, AnnotatedString annotatedString, TextStyle textStyle, List<AnnotatedString.Range<Placeholder>> list, int i2, boolean z, int i3, Density density, LayoutDirection layoutDirection, FontFamily.Resolver resolver, long j) {
        ...
        if (textLayoutResult.getMultiParagraph().getIntrinsics().getHasStaleResolvedFonts() || !m.c(layoutInput.getText(), annotatedString) || !layoutInput.getStyle().hasSameLayoutAffectingAttributes(textStyle) || !m.c(layoutInput.getPlaceholders(), list) || layoutInput.getMaxLines() != i2 || layoutInput.getSoftWrap() != z || !TextOverflow.m5212equalsimpl0(layoutInput.getOverflow(), i3) || !m.c(layoutInput.getDensity(), density) || layoutInput.getLayoutDirection() != layoutDirection || !m.c(layoutInput.getFontFamilyResolver(), resolver) || Constraints.m5237getMinWidthimpl(j) != Constraints.m5237getMinWidthimpl(layoutInput.getConstraints())) {
            return false;
        }
        if (z || TextOverflow.m5212equalsimpl0(i3, TextOverflow.INSTANCE.m5220getEllipsisgIe3tQ8())) {
            return Constraints.m5235getMaxWidthimpl(j) == Constraints.m5235getMaxWidthimpl(layoutInput.getConstraints()) && Constraints.m5234getMaxHeightimpl(j) == Constraints.m5234getMaxHeightimpl(layoutInput.getConstraints());
        }
        return true;
    }
}
