...
public final /* data */ class BiasAbsoluteAlignment implements Alignment {
    ...
    public long mo2476alignKFBX0sM(long size, long space, LayoutDirection layoutDirection) {
        m.h(layoutDirection, "layoutDirection");
        long jIntSize = IntSizeKt.IntSize(IntSize.m5427getWidthimpl(space) - IntSize.m5427getWidthimpl(size), IntSize.m5426getHeightimpl(space) - IntSize.m5426getHeightimpl(size));
        float f = 1;
        return IntOffsetKt.IntOffset(f.f0((this.horizontalBias + f) * (IntSize.m5427getWidthimpl(jIntSize) / 2.0f)), f.f0((f + this.verticalBias) * (IntSize.m5426getHeightimpl(jIntSize) / 2.0f)));
    }
