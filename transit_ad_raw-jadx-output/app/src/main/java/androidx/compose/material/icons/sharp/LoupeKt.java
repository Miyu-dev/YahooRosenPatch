package androidx.compose.material.icons.sharp;

import a.b;
import androidx.appcompat.R;
import androidx.appcompat.widget.s;
import androidx.browser.browseractions.a;
import androidx.compose.material.icons.Icons;
import androidx.compose.ui.graphics.Color;
import androidx.compose.ui.graphics.SolidColor;
import androidx.compose.ui.graphics.StrokeCap;
import androidx.compose.ui.graphics.StrokeJoin;
import androidx.compose.ui.graphics.vector.ImageVector;
import androidx.compose.ui.graphics.vector.PathBuilder;
import androidx.compose.ui.graphics.vector.VectorKt;
import androidx.compose.ui.unit.Dp;
import kotlin.Metadata;
import kotlin.jvm.internal.m;

/* JADX INFO: compiled from: Loupe.kt */
/* JADX INFO: loaded from: C:\ClaudeWork\YahoRsen\YahooRosenPatch\tmp\patching-1782388517898\patched\originalDex\classes.dex */
@Metadata(d1 = {"\u0000\u0010\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0003\"\u0010\u0010\u0000\u001a\u0004\u0018\u00010\u0001X\u0082\u000e¢\u0006\u0002\n\u0000\"\u0015\u0010\u0002\u001a\u00020\u0001*\u00020\u00038F¢\u0006\u0006\u001a\u0004\b\u0004\u0010\u0005¨\u0006\u0006"}, d2 = {"_loupe", "Landroidx/compose/ui/graphics/vector/ImageVector;", "Loupe", "Landroidx/compose/material/icons/Icons$Sharp;", "getLoupe", "(Landroidx/compose/material/icons/Icons$Sharp;)Landroidx/compose/ui/graphics/vector/ImageVector;", "material-icons-extended-sharp_release"}, k = 2, mv = {1, 8, 0}, xi = R.styleable.AppCompatTheme_checkboxStyle)
public final class LoupeKt {
    private static ImageVector _loupe;

    public static final ImageVector getLoupe(Icons.Sharp sharp) {
        m.h(sharp, "<this>");
        ImageVector imageVector = _loupe;
        if (imageVector != null) {
            m.e(imageVector);
            return imageVector;
        }
        ImageVector.Builder builder = new ImageVector.Builder("Sharp.Loupe", Dp.m5267constructorimpl(24.0f), Dp.m5267constructorimpl(24.0f), 24.0f, 24.0f, 0L, 0, false, 224, null);
        int defaultFillType = VectorKt.getDefaultFillType();
        SolidColor solidColor = new SolidColor(Color.INSTANCE.m2844getBlack0d7_KjU(), null);
        int iM3141getButtKaPHkGw = StrokeCap.INSTANCE.m3141getButtKaPHkGw();
        int iM3151getBevelLxFBmk8 = StrokeJoin.INSTANCE.m3151getBevelLxFBmk8();
        PathBuilder pathBuilderO = b.o(13.0f, 7.0f, -2.0f, 4.0f);
        b.s(pathBuilderO, 7.0f, 11.0f, 2.0f, 4.0f);
        a.l(pathBuilderO, 4.0f, 2.0f, -4.0f, 4.0f);
        s.x(pathBuilderO, -2.0f, -4.0f, 13.0f, 7.0f);
        pathBuilderO.moveTo(12.73f, 2.03f);
        pathBuilderO.curveToRelative(-6.08f, -0.44f, -11.14f, 4.62f, -10.7f, 10.7f);
        pathBuilderO.curveToRelative(0.38f, 5.28f, 5.0f, 9.27f, 10.29f, 9.27f);
        pathBuilderO.lineTo(22.0f, 22.0f);
        pathBuilderO.verticalLineToRelative(-9.68f);
        pathBuilderO.curveToRelative(0.0f, -5.3f, -3.98f, -9.91f, -9.27f, -10.29f);
        pathBuilderO.close();
        pathBuilderO.moveTo(12.0f, 20.0f);
        pathBuilderO.curveToRelative(-4.41f, 0.0f, -8.0f, -3.59f, -8.0f, -8.0f);
        pathBuilderO.reflectiveCurveToRelative(3.59f, -8.0f, 8.0f, -8.0f);
        pathBuilderO.reflectiveCurveToRelative(8.0f, 3.59f, 8.0f, 8.0f);
        ImageVector imageVectorBuild = ImageVector.Builder.m3359addPathoIyEayM$default(builder, androidx.appcompat.view.menu.a.j(pathBuilderO, -3.59f, 8.0f, -8.0f, 8.0f), defaultFillType, "", solidColor, 1.0f, null, 1.0f, 1.0f, iM3141getButtKaPHkGw, iM3151getBevelLxFBmk8, 1.0f, 0.0f, 0.0f, 0.0f, 14336, null).build();
        _loupe = imageVectorBuild;
        m.e(imageVectorBuild);
        return imageVectorBuild;
    }
}
